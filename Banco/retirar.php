...
try {
    ...
    if ($cuenta['saldo'] < $monto) {
        $pdo->rollBack();
        http_response_code(400);
        echo json_encode(['error' => 'Saldo insuficiente. Tu saldo actual es $' . number_format($cuenta['saldo'], 2) . '.']);
        exit;
    }
    ...
    $ins = $pdo->prepare("
        ...
        VALUES (?, 'retiro', ?, 'Retiro en efectivo', ?)
    ");
    ...
    $pdo->rollBack();
    http_response_code(500);
    echo json_encode(['error' => 'No se pudo completar el retiro. Intenta de nuevo.']);
}
